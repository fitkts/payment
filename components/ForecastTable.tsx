...
type SortableForecastEntry = ForecastEntry & { revenue: number };
type SortKeys = keyof SortableForecastEntry;
...
const ForecastTable: React.FC<ForecastTableProps> = ({ entries, onDeleteEntry, onUpdateEntry }) => {
  const [sortConfig, setSortConfig] = useState<SortConfigItem[]>([{ key: 'revenue', direction: 'descending' }]);

  const entriesWithRevenue: SortableForecastEntry[] = useMemo(() => 
    entries.map(entry => ({ ...entry, revenue: entry.classCount * entry.unitPrice })),
    [entries]
  );

  const sortedEntries = useMemo(() => {
    let sortableItems = [...entriesWithRevenue];
  ...
  const totalRevenue = entries.reduce((acc, entry) => acc + (entry.classCount * entry.unitPrice), 0);
      ...
      <table className="min-w-full divide-y divide-slate-200">
        <thead className="bg-slate-100">
          <tr>
            ...
            <th scope="col" className="px-6 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
              <button onClick={(e) => requestSort('revenue', e)} className="flex items-center">
                  예상 매출 (원)
        ...
        <tbody className="bg-white divide-y divide-slate-200">
          {sortedEntries.map((entry, index) => (
            <tr key={entry.id} className="hover:bg-slate-50 transition-colors">
              <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-slate-500">{index + 1}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">{entry.memberName}</td>
              <td className="px-6 py-4 whitespace-nowrap">
                <input
                  type="number"
                  value={entry.classCount}
                  onChange={(e) => onUpdateEntry(entry.id, 'classCount', parseInt(e.target.value) || 0)}
                  className="w-24 px-2 py-1 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 bg-slate-100 text-slate-800"
                  min="0"
                />
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <input
                  ...
                  onChange={(e) => onUpdateEntry(entry.id, 'unitPrice', parseCurrency(e.target.value))}
                  className="w-32 px-2 py-1 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 bg-slate-100 text-slate-800"
                />
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700 font-semibold">
                {(entry.classCount * entry.unitPrice).toLocaleString()}
              </td>
        ...
        <tfoot className="bg-slate-100 border-t-2 border-slate-300">
          <tr className="text-lg font-bold text-slate-800">
            <td colSpan={4} className="px-6 py-4 text-right">매출 합계</td>
            <td className="px-6 py-4 whitespace-nowrap">
